...
export const InputPanel: React.FC = () => {
	...
	const [companyList, setCompanyList] = useState(["ABC Logistics", "XYZ Corporation", "Fast Shipping Ltd.", "Tech Innovators", "Green Energy Co."]); // List of companies
	...
	const filteredCompanies = companyList.filter((company) => company.toLowerCase().includes(searchTerm.toLowerCase()));
							...
							<ul className="absolute w-full bg-white border border-gray-300 rounded-md mt-1 max-h-40 overflow-y-auto shadow-lg">
								{filteredCompanies.length > 0 ? (
									filteredCompanies.map((company, index) => (
										<li key={index} className="p-2 flex justify-between items-center hover:bg-gray-200 cursor-pointer">
											<span
												onClick={() => {
